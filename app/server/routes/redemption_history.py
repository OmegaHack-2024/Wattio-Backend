...
from server.models.redemption_history import (
...
async def add_redemption_data(
    redemption: RedemptionHistorySchema = Body(...),
    ...
    updated_user = await update_user(
        user_id, {"points": user["points"] - redemption.points_spent}
    )
    if not updated_user:
        return ErrorResponseModel(
            "An error occurred", 404, "There was an error updating the user points."
        )
    ...
    return ResponseModel(
        added_redemption, "Redemption added and transaction logged successfully."
    )
